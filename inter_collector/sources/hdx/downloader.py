"""Download dataset resources from HDX (Humanitarian Data Exchange).

Uses GET requests to download CSV, XLSX, and XLS files from
data.humdata.org resource URLs.  Each dataset produces:

  {code}_info.json   - Catalog snapshot (org, license, resource URLs)
  {code}.csv         - Data in CSV format (if available)
  {code}.xlsx        - Data in Excel format (if available)
  {code}.xls         - Data in legacy Excel format (if available)

When a dataset has multiple resources of the same format, filenames
are derived from the resource name (sanitised) or indexed as
{code}_0.csv, {code}_1.csv, etc.
"""

from __future__ import annotations

import asyncio
import json
import logging
from collections import Counter
from pathlib import Path

import httpx

from .catalog import FOLDER_STYLE_DISPLAY, HDXEntry
from ...download_utils import BytesCallback, DownloadResult, _download_file

log = logging.getLogger(__name__)

# Retry configuration for individual file downloads
_DOWNLOAD_MAX_RETRIES = 4          # total attempts per file
_DOWNLOAD_BACKOFF_BASE = 10.0      # seconds — progression: 10s, 20s, 40s, 80s
_INTER_FILE_DELAY = 0.5            # seconds between resource downloads within one dataset


async def _download_file_with_retry(
    client: httpx.AsyncClient,
    url: str,
    dest: Path,
    *,
    label: str = "",
    on_bytes: BytesCallback | None = None,
) -> int:
    """Download a file with automatic retry on transient errors.

    Retries on HTTP 429 (rate limit) and 5xx (server error) with
    exponential backoff.  Also retries on connection timeouts and
    network errors.  On exhausted retries, raises HTTPStatusError
    with a synthetic 429 response so callers can record the failure.
    """
    for attempt in range(1, _DOWNLOAD_MAX_RETRIES + 1):
        try:
            n = await _download_file(client, url, dest, on_bytes=on_bytes)
            return n
        except httpx.HTTPStatusError as e:
            if e.response.status_code == 429 or e.response.status_code >= 500:
                wait = _DOWNLOAD_BACKOFF_BASE * (2 ** (attempt - 1))
                log.warning("HTTP %d downloading %s — retry %d/%d in %.0fs",
                            e.response.status_code, label or url, attempt, _DOWNLOAD_MAX_RETRIES, wait)
                await asyncio.sleep(wait)
                continue
            raise
        except (httpx.TimeoutException, httpx.ConnectError):
            if attempt < _DOWNLOAD_MAX_RETRIES:
                await asyncio.sleep(_DOWNLOAD_BACKOFF_BASE * (2 ** (attempt - 1)))
                continue
            raise
    raise httpx.HTTPStatusError(
        f"Exhausted {_DOWNLOAD_MAX_RETRIES} retries for {label or url}",
        request=httpx.Request("GET", url), response=httpx.Response(429),
    )


async def download_dataset(
    client: httpx.AsyncClient,
    entry: HDXEntry,
    output_dir: Path,
    *,
    skip_existing: bool = True,
    on_bytes: BytesCallback | None = None,
    folder_style: str = FOLDER_STYLE_DISPLAY,
    download_formats: set[str] | None = None,
) -> DownloadResult:
    """Download all resources for a single HDX dataset.

    Creates this structure under output_dir / entry.get_folder_path(style):
        {code}_info.json   - Catalog snapshot (always written first)
        {code}.csv         - CSV data (if resource available)
        {code}.xlsx        - Excel data (if resource available)
        {code}.xls         - Legacy Excel data (if resource available)

    When multiple resources share the same format, filenames use the
    sanitised resource name or fall back to {code}_{index}.{ext}.

    Args:
        client:           Async HTTP client for downloads.
        entry:            HDXEntry with pre-filtered resource list.
        output_dir:       Root output directory (entry's folder_path appended).
        skip_existing:    Skip files already present on disk.
        on_bytes:         Callback for progress tracking (chunk_size, content_length).
        folder_style:     "display" for slugified names, "code" for raw codes.
        download_formats: If set, only download resources whose format matches.
    """
    code = entry.code
    dest_dir = output_dir / entry.get_folder_path(folder_style)
    dest_dir.mkdir(parents=True, exist_ok=True)

    result = DownloadResult(code)

    resources = entry.resources
    if download_formats:
        fmt_upper = {f.upper() for f in download_formats}
        resources = [r for r in resources if r["format"].upper() in fmt_upper]

    # 1. Save info snapshot
    info_file = dest_dir / f"{code}_info.json"
    if not (skip_existing and info_file.exists()):
        info = {
            "code": code,
            "title": entry.title,
            "description": entry.description[:500] if entry.description else "",
            "path": entry.full_path,
            "folder_path": str(entry.folder_path),
            "updated": entry.updated,
            "organization": entry.organization,
            "license_url": entry.license_url,
            "license_title": entry.license_title,
            "groups": entry.groups,
            "resources": [{"format": r["format"], "download_url": r["download_url"], "name": r.get("name", "")}
                          for r in resources],
        }
        data = json.dumps(info, indent=2, ensure_ascii=False).encode()
        info_file.write_bytes(data)
        result.bytes_downloaded += len(data)
        result.successes.append("info")

    # 2. Download data resources
    # Build filenames — use resource name if unique, else {code}.{ext}
    fmt_counts: Counter[str] = Counter(r["format"].upper() for r in resources)
    fmt_seen: Counter[str] = Counter()

    for i, res in enumerate(resources):
        fmt = res["format"].upper()
        ext = fmt.lower()

        if fmt_counts[fmt] > 1:
            # Multiple resources of same format — use resource name or index
            res_name = res.get("name", "")
            if res_name:
                safe_name = _safe_filename(res_name, ext)
            else:
                safe_name = f"{code}_{fmt_seen[fmt]}.{ext}"
            filename = safe_name
        else:
            filename = f"{code}.{ext}"

        fmt_seen[fmt] += 1
        dest_file = dest_dir / filename

        if skip_existing and dest_file.exists():
            continue

        if i > 0:
            await asyncio.sleep(_INTER_FILE_DELAY)

        try:
            n = await _download_file_with_retry(
                client, res["download_url"], dest_file,
                label=f"{code}/{filename}", on_bytes=on_bytes,
            )
            result.bytes_downloaded += n
            result.successes.append(ext)
            log.info("  [%s] %s OK (%d bytes)", ext.upper(), code, n)
        except Exception as e:
            result.failures.append((ext, str(e)))
            log.warning("  [%s] %s FAILED: %s", ext.upper(), code, e)

    return result


def _safe_filename(name: str, ext: str) -> str:
    """Convert a resource name to a filesystem-safe filename.

    Normalises Unicode to ASCII, lowercases, strips non-alphanumeric
    characters, and ensures the correct extension is appended.
    Truncates to 120 characters to stay within common filesystem limits
    (ext4: 255 bytes, NTFS: 255 chars, macOS HFS+: 255 chars).
    """
    import re
    import unicodedata
    s = unicodedata.normalize("NFKD", name)
    s = s.encode("ascii", "ignore").decode("ascii")
    s = s.lower()
    s = re.sub(r"[^a-z0-9._-]+", "_", s)
    s = re.sub(r"_+", "_", s).strip("_")
    if not s.endswith(f".{ext}"):
        s = f"{s}.{ext}"
    # Truncate to stay well within filesystem name limits
    if len(s) > 120:
        s = s[:120]
    return s or f"resource.{ext}"
